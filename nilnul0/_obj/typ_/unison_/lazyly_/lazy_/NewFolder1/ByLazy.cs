...
namespace nilnul._obj.typ_.nilable_.unprimable_.singleton_
{
	...
	///
	[Obsolete(nameof(ByLazy<YourClass>))]
	public	class ByLazy_threadUnsafe<YourClass>
		where YourClass : new()
		//where YourClass:class
	{
		...
		static private System.Lazy<YourClass> _Instance = new System.Lazy<YourClass>(
							//()=>unprimable_.Singleton<YourClass>.Instance
							//,
							nilnul._obj.typ_.unison_._LazylyX.MODE

		);
		static ByLazy_threadUnsafe()	//due to the "Intance" body has access to "_Instance", so "_Instance will be initalized". So maybe we can remove this static constructor?
		{ }
