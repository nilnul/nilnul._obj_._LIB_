...
namespace nilnul._obj.typ_.nilable_.unprimable_.singleton_
{
	...
	///
	[Obsolete(nameof(unison_.ILazyly) + " is denoted that we encounter some issues, and We don't quite get what MsDoc says;")]
	static public class ByLazy<YourClass>
		where YourClass : new()
		//where YourClass:class
	{
		...
		static public readonly System.Lazy<YourClass> _Instance =  new System.Lazy<YourClass>(
				() => new YourClass()	//this would execture once, and cached for later access
				,
				nilnul._obj.typ_.unison_._LazylyX.MODE

			//true /*The thread safety mode of a Lazy<T> instance that is initialized with this constructor is LazyThreadSafetyMode.ExecutionAndPublication if isThreadSafe is true*/
			);
		/*
